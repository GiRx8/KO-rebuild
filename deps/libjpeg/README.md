...
libraries can be controlled with the `LIBJPEG_BUILD_<SHARED|STATIC>_LIBS` CMake flags:
...
these tools, set the `LIBJPEG_BUILD_EXECUTABLES` flag:
```Shell
...
by using the `LIBJPEG_LINK_STATIC` flag:
```Shell
...
interface can be enabled with the `LIBJPEG_BUILD_ALT_UI` flag:
```Shell
...
`LIBJPEG_BUILD_EXECUTABLES` is enabled. They can be run using the `test` target:
```Shell
cmake ..
make
make test
