...
void CGameProcMain::ProcessLocalInput(uint32_t dwMouseFlags)
{
	...
	if (!IsUIKeyOperated() && nullptr == CN3UIBase::GetFocusedEdit()) // 채팅모드가 아닐때
	{
		...
		{
			// CommandToggleUIMiniMap(); // Disable old minimap toggle
		}
...
void CGameProcMain::InitUI()
{
	...
	m_pUICmd->SetVisible(false); // Hide old main command bar
	...
	m_pUIMsgDlg->SetVisibleWithNoSound(false); // Hide native info box output
	...
	m_pUIMsgDlg2->SetVisibleWithNoSound(false); // Hide native info box 2 output
	...
	m_pUIStateBarAndMiniMap->SetStyle(UISTYLE_FOCUS_UNABLE);
	m_pUIStateBarAndMiniMap->SetVisible(false);
#ifdef _DEBUG
...
{
	if (nullptr == m_pUIStateBarAndMiniMap)
		return;
