...
void CGameProcedure::Render()
{
	...
	CWebUI::Instance()->Render(); // Render Web UI overlay

	// Render inventory tooltips and destruction confirmation dialogs on top of WebUI overlay
	if (s_pProcMain && s_pProcMain->m_pUIInventory)
	{
		s_pProcMain->m_pUIInventory->RenderTooltipAndDestructionDialog();
	}
