...
bool CUIChat::Load(File& file)
{
	...
	// Hide all original chat window components including the input edit box (using HTML input instead)
...
void CUIChat::AddChatMsg(e_ChatMode eCM, const std::string& szString, D3DCOLOR color)
{
	...
	if (pWebUI)
	{
		pWebUI->AddGameMessage((int)eCM, szString, color);
	}
...
void CUIChat::SetFocus()
{
	...
	if (CN3UIEdit::s_hWndParent != nullptr)
	{
		::SetFocus(CN3UIEdit::s_hWndParent);
	}

	if (CWebUI::Instance()->GetView() != nullptr)
	{
		CWebUI::Instance()->GetView()->Focus();
	}
	CWebUI::Instance()->Evaluate("if (window.focusChatInput) { window.focusChatInput(); }");
}
...
void CUIChat::KillFocus()
{
	...
	if (CWebUI::Instance()->GetView() != nullptr)
	{
		CWebUI::Instance()->GetView()->Unfocus();
	}
	CWebUI::Instance()->Evaluate("if (window.blurChatInput) { window.blurChatInput(); }");
}
...
void CUIChat::SetEditBoxRegion(int x, int y, int w, int h)
{
	if (m_pEdit != nullptr)
	{
		m_pEdit->SetPos(x, y);
...
void CUIChat::RenderEditBox()
{
	if (m_pEdit != nullptr && m_pEdit->IsVisible() && IsChatMode())
	{
		m_pEdit->Render();
	}
}
