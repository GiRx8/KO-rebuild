...
{
	std::ofstream logFile("C:\\Users\\GiR\\Documents\\GitHub\\KnightOnline\\web_drag_log.txt", std::ios::app);
	if (logFile.is_open())
	{
...
{
	return; // Fully hidden, rendering is done in HTML5
}
...
void CUIHotKeyDlg::WebRegisterHotkey(__IconItemSkill* pItem, int iOrder)
{
	if (iOrder < 0 || iOrder >= MAX_SKILL_IN_HOTKEY || pItem == nullptr || pItem->pItemBasic == nullptr)
		return;
	if (m_iCurPage < 0 || m_iCurPage >= MAX_SKILL_HOTKEY_PAGE)
		return;
	...
	__TABLE_UPC_SKILL* pUSkill = CGameBase::s_pTbl_Skill.Find(pItem->pItemBasic->dwEffectID1);
	if (pUSkill == nullptr)
		return;
	...
	spSkill->pSkill = pUSkill;
	spSkill->szIconFN = fmt::format(
		"UI\\skillicon_{:02}_{}.dxt", pItem->pItemBasic->dwEffectID1 % 100, pItem->pItemBasic->dwEffectID1 / 100);
...
extern bool g_bHotkeyMovedRecently;

void CUIHotKeyDlg::WebMoveHotkey(int fromIndex, int toIndex)
{
	LogToFile(std::format("WebMoveHotkey started: fromIndex={}, toIndex={}", fromIndex, toIndex));

	if (fromIndex < 0 || fromIndex >= MAX_SKILL_IN_HOTKEY || toIndex < 0 || toIndex >= MAX_SKILL_IN_HOTKEY)
	{
		LogToFile("WebMoveHotkey: index out of bounds");
		return;
	}
	if (fromIndex == toIndex)
	{
		LogToFile("WebMoveHotkey: fromIndex == toIndex");
		return;
	}
	if (m_iCurPage < 0 || m_iCurPage >= MAX_SKILL_HOTKEY_PAGE)
	{
		LogToFile("WebMoveHotkey: m_iCurPage out of bounds");
		return;
	}

	__IconItemSkill* spSkillFrom = m_pMyHotkey[m_iCurPage][fromIndex];
	__IconItemSkill* spSkillTo = m_pMyHotkey[m_iCurPage][toIndex];

	LogToFile(std::format("WebMoveHotkey pointers: spSkillFrom={}, spSkillTo={}", (void*)spSkillFrom, (void*)spSkillTo));

	if (spSkillFrom)
	{
		LogToFile(std::format("spSkillFrom details: pUIIcon={}, pSkill={}", (void*)spSkillFrom->pUIIcon, (void*)spSkillFrom->pSkill));
	}
	if (spSkillTo)
	{
		LogToFile(std::format("spSkillTo details: pUIIcon={}, pSkill={}", (void*)spSkillTo->pUIIcon, (void*)spSkillTo->pSkill));
	}

	// Swap slots
	m_pMyHotkey[m_iCurPage][fromIndex] = spSkillTo;
	m_pMyHotkey[m_iCurPage][toIndex] = spSkillFrom;
	LogToFile("WebMoveHotkey: Swapped slots in array");

	// Update regions
	CN3UIArea* pAreaFrom = GetChildAreaByiOrder(UI_AREA_TYPE_SKILL_HOTKEY, fromIndex);
	CN3UIArea* pAreaTo = GetChildAreaByiOrder(UI_AREA_TYPE_SKILL_HOTKEY, toIndex);
	LogToFile(std::format("WebMoveHotkey areas: pAreaFrom={}, pAreaTo={}", (void*)pAreaFrom, (void*)pAreaTo));

	if (spSkillFrom && spSkillFrom->pUIIcon && pAreaTo)
	{
		LogToFile("Updating spSkillFrom region");
		spSkillFrom->pUIIcon->SetRegion(pAreaTo->GetRegion());
		spSkillFrom->pUIIcon->SetMoveRect(pAreaTo->GetRegion());
	}
	if (spSkillTo && spSkillTo->pUIIcon && pAreaFrom)
	{
		LogToFile("Updating spSkillTo region");
		spSkillTo->pUIIcon->SetRegion(pAreaFrom->GetRegion());
		spSkillTo->pUIIcon->SetMoveRect(pAreaFrom->GetRegion());
	}

	LogToFile("Calling SetHotKeyPage");
	// Update visibility/state
	SetHotKeyPage(m_iCurPage);

	LogToFile("Calling DisableCountStrDisplay");
	DisableCountStrDisplay();
	LogToFile("Calling DisableTooltipDisplay");
	DisableTooltipDisplay();
	LogToFile("Calling CloseIconRegistry");
	CloseIconRegistry();
	LogToFile("WebMoveHotkey completed successfully");
	g_bHotkeyMovedRecently = true;
}

void CUIHotKeyDlg::WebClearHotkey(int index)
{
	if (index < 0 || index >= MAX_SKILL_IN_HOTKEY)
		return;
	if (m_iCurPage < 0 || m_iCurPage >= MAX_SKILL_HOTKEY_PAGE)
		return;

	if (m_pMyHotkey[m_iCurPage][index] != nullptr)
	{
		__IconItemSkill* spSkill = m_pMyHotkey[m_iCurPage][index];
		if (spSkill->pUIIcon)
		{
			RemoveChild(spSkill->pUIIcon);
			spSkill->pUIIcon->Release();
		...
		delete spSkill;
		m_pMyHotkey[m_iCurPage][index] = nullptr;
	}
...
void CUIHotKeyDlg::WebRegisterHotkeyByID(int skillId, int iOrder)
{
	if (iOrder < 0 || iOrder >= MAX_SKILL_IN_HOTKEY)
		return;
	if (m_iCurPage < 0 || m_iCurPage >= MAX_SKILL_HOTKEY_PAGE)
		return;

	__TABLE_UPC_SKILL* pUSkill = CGameBase::s_pTbl_Skill.Find(skillId);
	if (pUSkill == nullptr)
		return;
	...
	spSkill->pSkill = pUSkill;
	spSkill->szIconFN = fmt::format(
		"UI\\skillicon_{:02}_{}.dxt", pUSkill->dwID % 100, pUSkill->dwID / 100);
