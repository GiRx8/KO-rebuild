...
void CUIInventory::WebShowItemTooltip(bool bSlot, int order)
{
	if (!m_pUITooltipDlg) return;

	__IconItemSkill* pItem = nullptr;
	if (bSlot)
	{
		if (order >= 0 && order < ITEM_SLOT_COUNT)
			pItem = m_pMySlot[order];
	}
	else
	{
		if (order >= 0 && order < MAX_ITEM_INVENTORY)
			pItem = m_pMyInvWnd[order];
	}
	...
	if (pItem != nullptr)
	{
		POINT pt;
		GetCursorPos(&pt);
		HWND hWnd = CN3Base::s_hWndBase;
		if (hWnd)
		{
			ScreenToClient(hWnd, &pt);
		}
		m_pUITooltipDlg->DisplayTooltipsEnable(pt.x, pt.y, pItem);
	}
	else
	{
...
void CUIInventory::WebHideItemTooltip()
{
	if (m_pUITooltipDlg)
	{
		m_pUITooltipDlg->DisplayTooltipsDisable();
...
{
	// Do nothing here. We draw the tooltip and destruction dialog in
	// RenderTooltipAndDestructionDialog() after WebUI is rendered so they appear on top.
}

void CUIInventory::RenderTooltipAndDestructionDialog()
{
	if (!m_bVisible)
		return;

	// Render the native tooltip dialog if it is visible
	if (m_pUITooltipDlg && m_pUITooltipDlg->IsVisible())
	{
		m_pUITooltipDlg->Render();
	}

	// Render the item destruction confirmation popup dialog if it is active
...
uint32_t CUIInventory::MouseProc(uint32_t dwFlags, const POINT& ptCur, const POINT& ptOld)
{
	...
	if (CWebUI::Instance()->IsUIPixelActive(ptCur.x, ptCur.y))
	{
...
bool CUIInventory::WebRClickItem(bool bSlot, int iOrder)
{
	if (m_eInvenState == INV_STATE_REPAIR)
		return false;
	...
	if (bSlot)
	{
		if (iOrder < 0 || iOrder >= ITEM_SLOT_COUNT) return false;
		spItem = m_pMySlot[iOrder];
	}
	else
	{
		if (iOrder < 0 || iOrder >= MAX_ITEM_INVENTORY) return false;
		spItem = m_pMyInvWnd[iOrder];
	}
	...
	m_bRBtnProcessing = true;
	bool bRet = InvOpsSomething(spItem);
	m_bRBtnProcessing = false;

	return bRet;
}

bool CUIInventory::WebMoveItem(bool bSrcSlot, int iSrcOrder, bool bDstSlot, int iDstOrder)
{
	if (m_eInvenState == INV_STATE_REPAIR)
		return false;
	...
	if (bSrcSlot)
	{
		if (iSrcOrder < 0 || iSrcOrder >= ITEM_SLOT_COUNT) return false;
		spItem = m_pMySlot[iSrcOrder];
	}
	else
	{
		if (iSrcOrder < 0 || iSrcOrder >= MAX_ITEM_INVENTORY) return false;
		spItem = m_pMyInvWnd[iSrcOrder];
	}
	...
	s_sSelectedIconInfo.UIWndSelect.UIWnd  = UIWND_INVENTORY;
	s_sSelectedIconInfo.UIWndSelect.iOrder = iSrcOrder;
	s_sSelectedIconInfo.pItemSelect        = spItem;
	if (bSrcSlot)
		s_sSelectedIconInfo.UIWndSelect.UIWndDistrict = UIWND_DISTRICT_INVENTORY_SLOT;
	else
	...
	if ((s_sSelectedIconInfo.UIWndSelect.UIWndDistrict == UIWND_DISTRICT_INVENTORY_SLOT) && bArm)
	{
		if (!m_pMySlot[iDestiOrder])
		{
			if (IsValidPosFromArmToArm(iDestiOrder))
			{
				s_sRecoveryJobInfo.UIWndSourceEnd.UIWndDistrict = UIWND_DISTRICT_INVENTORY_SLOT;
				s_sRecoveryJobInfo.UIWndSourceEnd.iOrder        = iDestiOrder;
				SendInvMsg(0x04, spItem->pItemBasic->dwID + spItem->pItemExt->dwID, iSrcOrder, iDestiOrder);
				bResult = true;
			}
		}
		else
		{
			if (IsValidPosFromArmToArm(iDestiOrder))
			{
				s_sRecoveryJobInfo.pItemTarget                    = m_pMySlot[iDestiOrder];
				s_sRecoveryJobInfo.UIWndTargetStart.UIWnd         = UIWND_INVENTORY;
				s_sRecoveryJobInfo.UIWndTargetStart.UIWndDistrict = UIWND_DISTRICT_INVENTORY_SLOT;
				s_sRecoveryJobInfo.UIWndTargetStart.iOrder        = iDestiOrder;
				s_sRecoveryJobInfo.UIWndTargetEnd.UIWnd           = UIWND_INVENTORY;

				if (IsValidPosFromArmToArmInverse(iSrcOrder))
				{
					s_sRecoveryJobInfo.UIWndSourceEnd.UIWndDistrict = UIWND_DISTRICT_INVENTORY_SLOT;
					s_sRecoveryJobInfo.UIWndSourceEnd.iOrder        = iDestiOrder;
					s_sRecoveryJobInfo.UIWndTargetEnd.UIWndDistrict = UIWND_DISTRICT_INVENTORY_SLOT;
					s_sRecoveryJobInfo.UIWndTargetEnd.iOrder        = iSrcOrder;
					SendInvMsg(0x04, spItem->pItemBasic->dwID + spItem->pItemExt->dwID, iSrcOrder, iDestiOrder);
					bResult = true;
				}
			}
	...
	else if ((s_sSelectedIconInfo.UIWndSelect.UIWndDistrict == UIWND_DISTRICT_INVENTORY_SLOT) && !bArm)
	{
		if (!m_pMyInvWnd[iDestiOrder])
		{
			s_sRecoveryJobInfo.UIWndSourceEnd.UIWndDistrict = UIWND_DISTRICT_INVENTORY_INV;
			s_sRecoveryJobInfo.UIWndSourceEnd.iOrder        = iDestiOrder;
			SendInvMsg(0x02, spItem->pItemBasic->dwID + spItem->pItemExt->dwID, iSrcOrder, iDestiOrder);
			bResult = true;
		}
		else
		{
			...
			{
				s_sRecoveryJobInfo.UIWndSourceEnd.UIWndDistrict = UIWND_DISTRICT_INVENTORY_INV;
				s_sRecoveryJobInfo.UIWndSourceEnd.iOrder        = iEmpty;
				SendInvMsg(0x02, spItem->pItemBasic->dwID + spItem->pItemExt->dwID, iSrcOrder, iEmpty);
				bResult = true;
			}
		}
	}
	else if ((s_sSelectedIconInfo.UIWndSelect.UIWndDistrict == UIWND_DISTRICT_INVENTORY_INV) && bArm)
	{
		if (!m_pMySlot[iDestiOrder])
		{
			if (IsValidPosFromInvToArm(iDestiOrder))
			{
				s_sRecoveryJobInfo.UIWndSourceEnd.UIWndDistrict = UIWND_DISTRICT_INVENTORY_SLOT;
				s_sRecoveryJobInfo.UIWndSourceEnd.iOrder        = iDestiOrder;
				SendInvMsg(0x01, spItem->pItemBasic->dwID + spItem->pItemExt->dwID, iSrcOrder, iDestiOrder);
				bResult = true;
			}
		}
		else
	...
	else if ((s_sSelectedIconInfo.UIWndSelect.UIWndDistrict == UIWND_DISTRICT_INVENTORY_INV) && !bArm)
	{
		if (!m_pMyInvWnd[iDestiOrder])
		{
			s_sRecoveryJobInfo.UIWndSourceEnd.UIWndDistrict = UIWND_DISTRICT_INVENTORY_INV;
			s_sRecoveryJobInfo.UIWndSourceEnd.iOrder        = iDestiOrder;
			SendInvMsg(0x03, spItem->pItemBasic->dwID + spItem->pItemExt->dwID, iSrcOrder, iDestiOrder);
			bResult = true;
		}
		else
		{
			s_sRecoveryJobInfo.UIWndSourceEnd.UIWndDistrict   = UIWND_DISTRICT_INVENTORY_INV;
			s_sRecoveryJobInfo.UIWndSourceEnd.iOrder          = iDestiOrder;
			s_sRecoveryJobInfo.pItemTarget                    = m_pMyInvWnd[iDestiOrder];
			s_sRecoveryJobInfo.UIWndTargetStart.UIWnd         = UIWND_INVENTORY;
			s_sRecoveryJobInfo.UIWndTargetStart.UIWndDistrict = UIWND_DISTRICT_INVENTORY_INV;
			s_sRecoveryJobInfo.UIWndTargetStart.iOrder        = iDestiOrder;
			s_sRecoveryJobInfo.UIWndTargetEnd.UIWnd           = UIWND_INVENTORY;
			s_sRecoveryJobInfo.UIWndTargetEnd.UIWndDistrict   = UIWND_DISTRICT_INVENTORY_INV;
			s_sRecoveryJobInfo.UIWndTargetEnd.iOrder          = iSrcOrder;
	...
	if (bResult)
	{
		...
		if (s_sRecoveryJobInfo.pItemSource != nullptr)
		{
			__IconItemSkill* spItemSrc = s_sRecoveryJobInfo.pItemSource;
			switch (s_sRecoveryJobInfo.UIWndSourceEnd.UIWndDistrict)
			{
				case UIWND_DISTRICT_INVENTORY_SLOT:
					pArea = GetChildAreaByiOrder(UI_AREA_TYPE_SLOT, s_sRecoveryJobInfo.UIWndSourceEnd.iOrder);
					m_pMySlot[s_sRecoveryJobInfo.UIWndSourceEnd.iOrder] = spItemSrc;
					if (pArea != nullptr && m_pMySlot[s_sRecoveryJobInfo.UIWndSourceEnd.iOrder]->pUIIcon != nullptr)
					{
						m_pMySlot[s_sRecoveryJobInfo.UIWndSourceEnd.iOrder]->pUIIcon->SetRegion(pArea->GetRegion());
						m_pMySlot[s_sRecoveryJobInfo.UIWndSourceEnd.iOrder]->pUIIcon->SetMoveRect(pArea->GetRegion());
					}
				...
				case UIWND_DISTRICT_INVENTORY_INV:
					pArea = GetChildAreaByiOrder(UI_AREA_TYPE_INV, s_sRecoveryJobInfo.UIWndSourceEnd.iOrder);
					m_pMyInvWnd[s_sRecoveryJobInfo.UIWndSourceEnd.iOrder] = spItemSrc;
					if (pArea != nullptr && m_pMyInvWnd[s_sRecoveryJobInfo.UIWndSourceEnd.iOrder]->pUIIcon != nullptr)
					{
						m_pMyInvWnd[s_sRecoveryJobInfo.UIWndSourceEnd.iOrder]->pUIIcon->SetRegion(pArea->GetRegion());
						m_pMyInvWnd[s_sRecoveryJobInfo.UIWndSourceEnd.iOrder]->pUIIcon->SetMoveRect(pArea->GetRegion());
					}
		...
		if (s_sRecoveryJobInfo.pItemTarget != nullptr)
		{
			__IconItemSkill* spItemTgt = s_sRecoveryJobInfo.pItemTarget;
			switch (s_sRecoveryJobInfo.UIWndTargetEnd.UIWndDistrict)
			{
				case UIWND_DISTRICT_INVENTORY_SLOT:
					pArea = GetChildAreaByiOrder(UI_AREA_TYPE_SLOT, s_sRecoveryJobInfo.UIWndTargetEnd.iOrder);
					m_pMySlot[s_sRecoveryJobInfo.UIWndTargetEnd.iOrder] = spItemTgt;
					if (pArea != nullptr && m_pMySlot[s_sRecoveryJobInfo.UIWndTargetEnd.iOrder]->pUIIcon != nullptr)
					{
						m_pMySlot[s_sRecoveryJobInfo.UIWndTargetEnd.iOrder]->pUIIcon->SetRegion(pArea->GetRegion());
						m_pMySlot[s_sRecoveryJobInfo.UIWndTargetEnd.iOrder]->pUIIcon->SetMoveRect(pArea->GetRegion());
					}
				...
				case UIWND_DISTRICT_INVENTORY_INV:
					pArea = GetChildAreaByiOrder(UI_AREA_TYPE_INV, s_sRecoveryJobInfo.UIWndTargetEnd.iOrder);
					m_pMyInvWnd[s_sRecoveryJobInfo.UIWndTargetEnd.iOrder] = spItemTgt;
					if (pArea != nullptr && m_pMyInvWnd[s_sRecoveryJobInfo.UIWndTargetEnd.iOrder]->pUIIcon != nullptr)
					{
						m_pMyInvWnd[s_sRecoveryJobInfo.UIWndTargetEnd.iOrder]->pUIIcon->SetRegion(pArea->GetRegion());
						m_pMyInvWnd[s_sRecoveryJobInfo.UIWndTargetEnd.iOrder]->pUIIcon->SetMoveRect(pArea->GetRegion());
					}
...
bool CUIInventory::WebDestroyItem(bool bSlot, int iOrder)
{
	if (m_eInvenState == INV_STATE_REPAIR)
		return false;
