...
{
	if (iValue < 0 || iValue >= 9) return;

	// 스킬창의 값..
	int iSkillExtra = m_iSkillInfo[0];
	int iSkillPoint = m_iSkillInfo[iValue];
	...
	iSkillPoint++;
	m_iSkillInfo[iValue] = iSkillPoint;

	// 동기화를 위해 native UI string도 존재하면 업데이트
	CN3UIString* pStrName = GetChildByID<CN3UIString>("string_skillpoint");
	if (pStrName) pStrName->SetStringAsInt(iSkillExtra);

	std::string szStrName2 = "";
	if (iValue >= 1 && iValue <= 8)
	{
		if (iValue == 1) szStrName2 = "string_0";
		else if (iValue == 2) szStrName2 = "string_1";
		else if (iValue == 3) szStrName2 = "string_2";
		else if (iValue == 4) szStrName2 = "string_3";
		else if (iValue == 5) szStrName2 = "string_4";
		else if (iValue == 6) szStrName2 = "string_5";
		else if (iValue == 7) szStrName2 = "string_6";
		else if (iValue == 8) szStrName2 = "string_7";
	}
	if (!szStrName2.empty())
	{
		CN3UIString* pStrName2 = GetChildByID<CN3UIString>(szStrName2);
		if (pStrName2) pStrName2->SetStringAsInt(iSkillPoint);
	}
