...
bool CUIStateBar::Load(File& file)
{
	...
	// Hide all original statebar widgets completely since everything is now in HTML5
...
void CUIStateBar::Tick()
{
	...
	// Dynamically align the mini-map group and buttons to the top-right of the viewport
	if (m_pGroup_MiniMap != nullptr)
	{
		RECT rc = m_pGroup_MiniMap->GetRegion();
		int w = rc.right - rc.left;
		int targetX = s_CameraData.vp.Width - w;
		int targetY = 0;
		POINT pos = m_pGroup_MiniMap->GetPos();
		if (pos.x != targetX || pos.y != targetY)
		{
			int dx = targetX - pos.x;
			int dy = targetY - pos.y;

			m_pGroup_MiniMap->SetPos(targetX, targetY);

			if (m_pBtn_Quest != nullptr) m_pBtn_Quest->SetPos(m_pBtn_Quest->GetPos().x + dx, m_pBtn_Quest->GetPos().y + dy);
			if (m_pBtn_Power != nullptr) m_pBtn_Power->SetPos(m_pBtn_Power->GetPos().x + dx, m_pBtn_Power->GetPos().y + dy);
			if (m_pText_Position != nullptr) m_pText_Position->SetPos(m_pText_Position->GetPos().x + dx, m_pText_Position->GetPos().y + dy);
		}
	}
