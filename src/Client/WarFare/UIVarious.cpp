...
void CUIVarious::Render()
{
	// Override and do nothing to prevent the old C++ status bar pages / tabs
	// from rendering on screen, keeping only the WebUI representation visible.
}

std::string CUIKnights::GetMemberListJson()
{
	std::string json = "[";
	bool first = true;
	for (auto& kmi : m_MemberList)
	{
		if (!first) json += ",";
		first = false;

		std::string escapedName = "";
		for (char c : kmi.szName)
		{
			if (c == '"' || c == '\\') escapedName += '\\';
			escapedName += c;
		}
		...
		json += fmt::format(
			"{{\"name\":\"{}\",\"duty\":{},\"level\":{},\"class\":{},\"connected\":{}}}",
			escapedName, static_cast<int>(kmi.eDuty), kmi.iLevel, static_cast<int>(kmi.eClass), kmi.iConnected
		);
	}
	json += "]";
	return json;
