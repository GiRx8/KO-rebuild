...
static void SetBasePathFromUIF(const char* pszPathName)
{
	if (nullptr == pszPathName)
		return;
	std::string strPath = pszPathName;
	std::string strPathLower = strPath;
	CharLower(&(strPathLower[0]));

	size_t pos = std::string::npos;
	if ((pos = strPathLower.find("\\ui_us\\")) != std::string::npos)
	{
		strPath = strPath.substr(0, pos);
	}
	else if ((pos = strPathLower.find("\\ui\\")) != std::string::npos)
	{
		strPath = strPath.substr(0, pos);
	}
	else if ((pos = strPathLower.find("\\chrselect\\")) != std::string::npos)
	{
		strPath = strPath.substr(0, pos);
	}
	else
	{
		// Fallback: go up two levels (i.e. parent of the file's folder)
		size_t lastBS = strPath.rfind('\\');
		if (lastBS != std::string::npos)
		{
			std::string strDir = strPath.substr(0, lastBS);
			size_t secondLastBS = strDir.rfind('\\');
			if (secondLastBS != std::string::npos)
			{
				strPath = strDir.substr(0, secondLastBS);
			}
		}
	}
	...
	if (pFrm)
	{
		pFrm->SetBasePath(strPath.c_str());
	}
}
...
BOOL CUIEDoc::OnOpenDocument(LPCTSTR lpszPathName)
{
	...
	SetBasePathFromUIF(lpszPathName);
